...
int* matchingStrings(int stringList_count, char** stringList, int queries_count, char** queries, int* result_count) {
    // Set the result count to be the same as the number of queries
    *result_count = queries_count;
    ...
    // For each query string, count its occurrences in the string list
    for (int i = 0; i < queries_count; i++) {
        for (int j = 0; j < stringList_count; j++) {
            // Compare the current query with the current string from the list
            if (strcmp(queries[i], stringList[j]) == 0) {
                // Increment the count if strings match
                result[i]++;
            }
        }
    }
