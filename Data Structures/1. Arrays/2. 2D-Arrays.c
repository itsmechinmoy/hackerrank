...
int hourglassSum(int arr_rows, int arr_columns, int** arr) {
    ...
    // This ensures any valid hourglass sum will be greater than this initial value
    int max_sum = INT_MIN;
    
    // Iterate through the 2D array to find all possible hourglasses
    // Since an hourglass needs 3 rows and 3 columns, we only need to check up to (rows-2) and (columns-2)
    for (int i = 0; i <= arr_rows - 3; i++) {
        for (int j = 0; j <= arr_columns - 3; j++) {
            // Calculate the sum of the current hourglass
            int current_sum = arr[i][j] + arr[i][j+1] + arr[i][j+2]  // Top row
